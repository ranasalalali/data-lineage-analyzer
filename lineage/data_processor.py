...
def get_missing_columns_impact(df1, df2):
    ...
    for table in distinct_tables:
        source_info = df2[df2["Source Table"].str.endswith(table.lower(), na=False)][
            ["Source Column", "Target Table", "Transformation"]
        ].dropna()
        existing_columns = (
            df1[df1["TableName"] == table]["ColumnName"].dropna().unique()
        )
        ...
        for _, row in source_info.iterrows():
            ...
            if (
                " " not in source_column
                and "(" not in source_column
                and "." not in source_column
            ):
                ...
                if (
                    ...
                    missing_columns[target_table].append(
                        {
